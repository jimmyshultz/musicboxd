...
const getSupabaseConfig = () => {
  ...
  if (Environment.isStaging) {
    ...
    // config.url = ENV_CONFIG.SUPABASE_STAGING_URL;
    // config.anonKey = ENV_CONFIG.SUPABASE_STAGING_ANON_KEY;
    Logger.log('Using staging Supabase configuration');
  } else if (Environment.isProduction) {
    // config.url = ENV_CONFIG.SUPABASE_PRODUCTION_URL;
    // config.anonKey = ENV_CONFIG.SUPABASE_PRODUCTION_ANON_KEY;
