...
try {
  console.log(
    '🍎 [DEBUG] Attempting to import Apple Authentication library...',
  );
  const appleAuthModule = require('@invertase/react-native-apple-authentication');
  console.log(
    '🍎 [DEBUG] Apple Authentication module imported:',
    Object.keys(appleAuthModule),
  );
  appleAuth = appleAuthModule.appleAuth;
  console.log(
    '🍎 [DEBUG] appleAuth object:',
    typeof appleAuth,
    appleAuth ? 'available' : 'null',
  );
} catch (error) {
  console.log(
    '🍎 [DEBUG] Apple Authentication library import failed:',
    error.message,
  );
...
export class AuthService {
  ...
  static initializeGoogleSignIn() {
    GoogleSignin.configure({
      ...
      iosClientId:
        '148204198310-f563ltpvfnibugfc3e3c9quaupnejb17.apps.googleusercontent.com', // iOS client ID
      webClientId:
        '148204198310-jb85bku6g5sdoggvt6idqq3j31momvl7.apps.googleusercontent.com', // Web client ID
    ...
    try {
      ...
      if (data.user) {
        ...
        if (!profile) {
          ...
          console.log(
            'Keeping existing user profile data - not overwriting avatar_url',
          );
        }
      }
  ...
  static async signInWithApple() {
    try {
      ...
      if (!appleAuth) {
        throw new Error(
          'Apple Authentication library not imported. Please check the installation.',
        );
      }

      if (!appleAuth.native) {
        throw new Error(
          'Apple Authentication native module not linked. The app needs to be rebuilt after pod install.',
        );
      }

      if (typeof appleAuth.performRequest !== 'function') {
        throw new Error(
          'Apple Authentication performRequest method not available. Please rebuild the app.',
        );
      }
      ...
      if (fullName?.givenName || fullName?.familyName) {
        ...
        email &&
        email.includes('@') &&
        !email.includes('appleid.private')
      ) {
        ...
        console.log(
          '🍎 [DEBUG] No name data available, trying to decode identity token',
        );
        ...
        try {
          ...
          if (
            ...
            !decodedPayload.email.includes('appleid.private')
          ) {
            ...
            if (emailPrefix && emailPrefix !== 'apple') {
              ...
              console.log(
                '🍎 [DEBUG] Using email prefix from token:',
                displayName,
              );
            }
          }
          ...
          console.log(
            '🍎 [DEBUG] Could not decode identity token:',
            tokenError,
          );
        }
  ...
  static async isAppleSignInAvailable(): Promise<boolean> {
    try {
      ...
      if (appleAuth) {
        ...
        console.log(
          '🍎 [DEBUG] isAvailableAsync exists:',
          typeof appleAuth.isAvailableAsync,
        );
      }
      ...
      if (!appleAuth.native) {
        ...
        console.log(
          '🍎 [DEBUG] - appleAuth.native exists:',
          !!appleAuth.native,
        );
        console.log(
          '🍎 [DEBUG] This means pod install worked but app needs rebuild',
        );
        return false;
      }
      ...
      if (typeof appleAuth.isAvailableAsync !== 'function') {
        ...
        if (Platform.OS === 'ios') {
          console.log(
            '🍎 [DEBUG] Fallback: Assuming available on iOS since native module exists',
          );
      ...
      console.error(
        '🍎 [DEBUG] Error checking Apple Sign-In availability:',
        error,
      );
      return false;
    }
  ...
  static async signOut() {
    try {
      ...
      try {
        ...
        console.log(
          'Could not get current user for push token deactivation:',
          userError,
        );
      }
