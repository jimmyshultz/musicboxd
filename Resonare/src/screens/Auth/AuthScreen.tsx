...
try {
  ...
  console.log(
    '🍎 [DEBUG] AuthScreen: Apple Auth module keys:',
    Object.keys(appleAuthModule),
  );
  AppleButton = appleAuthModule.AppleButton;
  console.log(
    '🍎 [DEBUG] AuthScreen: AppleButton imported:',
    typeof AppleButton,
    AppleButton ? 'available' : 'null',
  );
} catch (error) {
  console.log(
    '🍎 [DEBUG] AuthScreen: Apple Authentication UI components not available:',
    error.message,
  );
}
...
export default function AuthScreen() {
  ...
  useEffect(() => {
    ...
    const checkAppleSignInAvailability = async () => {
      try {
        console.log(
          '🍎 [DEBUG] AuthScreen: Starting Apple Sign-In availability check...',
        );
        console.log('🍎 [DEBUG] AuthScreen: Platform.OS:', Platform.OS);
        console.log(
          '🍎 [DEBUG] AuthScreen: AppleButton available:',
          !!AppleButton,
        );

        const available = await AuthService.isAppleSignInAvailable();
        console.log(
          '🍎 [DEBUG] AuthScreen: Apple Sign-In availability result:',
          available,
        );
        setIsAppleSignInAvailable(available);

        console.log(
          '🍎 [DEBUG] AuthScreen: State updated - isAppleSignInAvailable:',
          available,
        );
      } catch (availabilityError) {
        console.log(
          '🍎 [DEBUG] AuthScreen: Error checking Apple Sign-In availability:',
          availabilityError,
        );
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      console.log(
        '🔍 [DEBUG] AuthScreen: signInWithGoogle completed successfully',
      );
    } catch (signInError: any) {
      console.log(
        '🔍 [DEBUG] AuthScreen: signInWithGoogle failed:',
        signInError,
      );
      console.log('🔍 [DEBUG] AuthScreen: Error message:', signInError.message);
      console.log(
        '🔍 [DEBUG] AuthScreen: Full error object:',
  ...
  const handleAppleSignIn = async () => {
    try {
      ...
      console.log(
        '🍎 [DEBUG] AuthScreen: signInWithApple completed successfully',
      );
    } catch (signInError: any) {
      console.log(
        '🍎 [DEBUG] AuthScreen: signInWithApple failed:',
        signInError,
      );
      console.log('🍎 [DEBUG] AuthScreen: Error message:', signInError.message);
      console.log(
        ...
        <Card style={styles.authCard} elevation={2}>
          <Card.Content>
            ...
            <Text variant="bodySmall" style={styles.description}>
              Sign up to track your listening history, rate albums, and connect
            ...
            {Platform.OS === 'ios' && !AppleButton && (
              <>
                ...
                <Text variant="bodySmall" style={styles.infoText}>
                  Apple Sign-In will be available after running: cd ios && pod
                  install
                </Text>
              </>
            )}
