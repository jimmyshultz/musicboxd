...
type DiaryScreenRouteProp = RouteProp<
  ProfileStackParamList | HomeStackParamList | SearchStackParamList,
  'Diary'
>;
type DiaryScreenNavProp = StackNavigationProp<
  ProfileStackParamList | HomeStackParamList | SearchStackParamList
>;
...
export default function DiaryScreen() {
  ...
  const diaryState = useSelector(
    (state: RootState) => state.diary.byUserId[userId],
  );
  ...
  const groupIntoSections = useCallback((entries: DiaryEntry[]) => {
    ...
    entries.forEach(entry => {
      ...
      const monthHeader = d.toLocaleString(undefined, {
        month: 'long',
        year: 'numeric',
      });
  ...
  const loadInitial = useCallback(async () => {
    ...
    try {
      const { entries, lastMonth, hasMore } =
        await diaryEntriesService.getDiaryEntriesByUser(userId, {
          monthWindow: 3,
        });
  ...
  const renderRow = ({ item }: { item: DiaryEntry }) => {
    ...
    return (
      <TouchableOpacity
        style={styles.row}
        onPress={() =>
          navigation.navigate('DiaryEntryDetails' as any, {
            entryId: item.id,
            userId,
          })
        }
        ...
        <View style={styles.center}>
          {currentUser?.id === userId ? (
            <Text style={styles.emptyStateText}>
              No diary entries yet. Mark albums as listened and add them to your
              diary.
            </Text>
          ) : (
